...
const ACEITE_CAT = {
  ...
  HIDROLINA: "CAT-HIDROLINA",
};
const unitOf = (cat) =>
  cat === "CAT-ACEITE" || cat === "CAT-LIQUIDO" || cat === "CAT-REFRIG" || cat === "CAT-HIDROLINA"
    ? "LT"
    : cat === "CAT-GRASA"
      ? "KG"
      : "UND";
...
function tiposOf(name) {
  const n = norm(name);
  const s = new Set();
  for (const [tok, tipo] of EQUIP) {
    if (new RegExp(`\\b${tok}\\b`).test(n)) s.add(tipo);
  }
  return [...s];
}
...
for (const file of files) {
  ...
  for (let r = 1; r <= 15; r++) {
    const c = ws
      ...
      .values.map(txt)
      .map((s) => s.toUpperCase());
    if (c.includes("CÓDIGO") && c.includes("PRODUCTO")) {
      hr = r;
      break;
    }
  }
  const idx = {};
  ws.getRow(hr).eachCell((c, i) => {
    const t = txt(c.value).toUpperCase();
    if (t && !(t in idx)) idx[t] = i;
  });
  const find = (re) => {
    for (const [k, i] of Object.entries(idx)) if (re.test(k)) return i;
    return -1;
  };
  const cCode = find(/^CÓDIGO$/),
    cName = find(/^PRODUCTO$/),
    cCat = find(/CATEGOR/);
  for (let r = hr + 1; r <= ws.rowCount; r++) {
    const row = ws.getRow(r);
    const sku = txt(row.getCell(cCode).value);
    const name = txt(row.getCell(cName).value);
    if (!sku || !name || sku.toUpperCase() === name.toUpperCase() || sku.toUpperCase() === "CÓDIGO")
      continue;
    ...
    let cat = fam;
    if (fam === "FAM-LUB") {
      const cv = cCat > 0 ? norm(txt(row.getCell(cCat).value)) : "";
      cat = ACEITE_CAT[cv] || "FAM-LUB";
    }
    const tipos = tiposOf(name);
    const esGen = tipos.length === 0;
...
for (const t of [
  "T_OrdenMantenimientoTrabajo",
  "T_OrdenMantenimiento",
  "T_ProductoPrecioHistorico",
  "T_SaldoStock",
  "T_RequerimientoDetalle",
  "T_Requerimiento",
  "T_DocumentoInventarioDetalle",
  "T_DocumentoInventario",
  "T_ProductoImagen",
  "T_ProductoTipoEquipo",
  "T_Importacion",
  "T_Producto",
])
  sql += `DELETE FROM "inv"."${t}";\n`;
...
sql +=
  [
    ["FAM-HER", "Herramientas"],
    ["FAM-FIL", "Filtros"],
    ["FAM-ESL", "Eslingas y Grilletes"],
    ["FAM-SUS", "Sistema de Suspensión"],
    ["FAM-SUM", "Suministros de Rotación"],
    ["FAM-LUB", "Aceites y Líquidos"],
  ]
    .map(([c, n]) => `  ('${c}','${esc(n)}',NULL)`)
    .join(",\n") + ";\n\n";
...
sql += productos
  .map((p) => `  ('${esc(p.sku)}','${esc(p.name)}','${p.cat}','${p.uni}',${p.esGen})`)
  .join(",\n");
...
console.log(
  ...
  productos.length,
  "| Generales:",
  productos.filter((p) => p.esGen).length,
  "| Específicos:",
  productos.filter((p) => !p.esGen).length,
  "| Asociaciones:",
  assoc.length,
